...
public class UnIgnoreCommand {
	...
	@Command(names = {"unignore", "unblock"}, permission = "")
	public static void ignore(Player sender, @Param(name = "player") Profile target) {
		...
		if (!senderProfile.getIgnored().contains(target.getUuid())) {
			sender.sendMessage(CC.translate("&cYou don't have that player ignored."));
			return;
		}

		senderProfile.getIgnored().remove(target.getUuid());
		senderProfile.save();

		sender.sendMessage(CC.translate("&a" + target.getUsername() + " has been successfully removed from your ignored."));
