...
@AllArgsConstructor
@Data
public class StaffQuitPacket implements XPacket {
	...
	public void onReceive() {
		Bukkit.getConsoleSender().sendMessage(CC.translate("&9[S] &r" + sender + " &bhas just left &7" + server));

		Bukkit.getOnlinePlayers()
				.stream()
				.filter(player -> player.hasPermission("gravity.staff"))
				.forEach(player -> {
					player.sendMessage(CC.translate("&9[S] &r" + sender + " &bhas just left &7" + server));
				});
