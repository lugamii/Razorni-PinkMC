...
public class UserAddPermissionCommand {

    @Command(names = "user addperm", permission = "gravity.command.user", async = true, description = "Adds a permission to a players profile.")
    public static void removePerm(CommandSender sender, @Param(name = "player") UUID uuid, @Param(name = "permission") String permission) {
        ...
        profile.save();
        sender.sendMessage(CC.GREEN + "Added permission " + "\'" + permission + "\' for " + profile.getUsername() + '.');
    }
