...
public class LootBoxEditItemsButton extends Button {
    ...
    public ItemStack getItem(Player player) {
        return new ItemBuilder(Material.CHEST)
                ...
                .setLore(Arrays.asList(
                        CC.MENU_BAR,
                        ChatColor.WHITE + "Click to edit lootbox items.",
                        CC.MENU_BAR)
                ).build();
    }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                plugin.getLootBoxManager()
                        .getLootBoxEditingMap().put(player.getUniqueId(), lootBox);

                Inventory inventory = Bukkit.createInventory(null, 54, lootBox.getDisplayName());

                AtomicInteger slot = new AtomicInteger();
                lootBox.getItems().forEach(lootBoxItem -> {
