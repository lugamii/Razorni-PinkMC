...
import dev.razorni.hcfactions.utils.commandapi.Stopwatch;
import dev.razorni.hcfactions.utils.commandapi.fanciful.FancyMessage;
...
public class CommandNode {
    ...
    public CommandNode getCommand(String name) {
        return (CommandNode)this.children.get(name.toLowerCase());
    }
    ...
    public FancyMessage getUsage(String realLabel) {
        ...
        flags.addAll((Collection)this.parameters.stream().filter((datax) -> {
            return datax instanceof FlagData;
        }).map((datax) -> {
            return (FlagData)datax;
        }).collect(Collectors.toList()));
        List<ParameterData> parameters = Lists.newArrayList();
        parameters.addAll((Collection)this.parameters.stream().filter((datax) -> {
        ...
        if (!flags.isEmpty()) {
            usage.then("(").color(ChatColor.GOLD);
            if (!Strings.isNullOrEmpty(this.getDescription())) {
                usage.tooltip(ChatColor.YELLOW + this.getDescription());
            }

            for (FlagData data : flags) {
                String name = (String) data.getNames().get(0);
                if (!flagFirst) {
                    usage.then(" | ").color(ChatColor.GOLD);
                    if (!Strings.isNullOrEmpty(this.getDescription())) {
                        usage.tooltip(ChatColor.YELLOW + this.getDescription());
                ...
                flagFirst = false;
                usage.then("-" + name).color(ChatColor.WHITE);
        ...
        if (!parameters.isEmpty()) {
            for(int index = 0; index < parameters.size(); ++index) {
                ParameterData data = (ParameterData)parameters.get(index);
    ...
    public FancyMessage getUsage() {
        ...
        if (!flags.isEmpty()) {
            ...
            Iterator<FlagData> var5 = flags.iterator();

            while(var5.hasNext()) {
                FlagData data = var5.next();
                String name = (String)data.getNames().get(0);
                if (!flagFirst) {
                    usage.then(" | ").color(ChatColor.RED);
                }
        ...
        if (!parameters.isEmpty()) {
            for(int index = 0; index < parameters.size(); ++index) {
                ParameterData data = (ParameterData)parameters.get(index);
                boolean required = data.getDefaultValue().isEmpty();
                usage.then((required ? "<" : "[") + data.getName() + (data.isWildcard() ? "..." : "") + (required ? ">" : "]") + (index != parameters.size() - 1 ? " " : "")).color(ChatColor.GOLD);
            }
        }
    ...
    public boolean invoke(CommandSender sender, Arguments arguments) throws CommandException {
        if (this.method == null) {
            ...
            objects.add(sender);
            int index = 0;
            Iterator<Data> var5 = this.parameters.iterator();

            while(true) {
                while(var5.hasNext()) {
                    Data unknownData = var5.next();
                    if (unknownData instanceof FlagData) {
                        FlagData flagData = (FlagData)unknownData;
                        boolean value = flagData.getDefaultValue();
                        Iterator<String> var18 = flagData.getNames().iterator();
                        ...
                        String argument;
                        try {
                            argument = (String)arguments.getArguments().get(index);
                ...
                try {
                    ...
                    stopwatch.start();
                    this.method.invoke((Object)null, objects.toArray());
                    stopwatch.stop();
                    int executionThreshold = 10;
                    if (!this.async && this.logToConsole && stopwatch.elapsedMillis() >= (long)executionThreshold) {
                        HCF.getPlugin().getLogger().warning("Command '/" + this.getFullLabel() + "' took " + stopwatch.elapsedMillis() + "ms!");
                    }
    ...
    public List<String> getSubCommands(CommandSender sender, boolean print) {
        ...
        if (this.canUse(sender)) {
            ...
            if (this.hasCommands()) {
                Iterator<CommandNode> var5 = this.getChildren().values().iterator();

                while(var5.hasNext()) {
                    CommandNode n = var5.next();
        ...
        if (!commands.isEmpty() && print) {
            sender.sendMessage(ChatColor.GOLD.toString() + ChatColor.STRIKETHROUGH + StringUtils.repeat("-", 35));

            for (String command : commands) {
                sender.sendMessage(ChatColor.GRAY + "┃ " + ChatColor.WHITE + command);
            }
