...
public class QueueHandler {
    ...
    public QueueHandler() {
        for(String queue : Hub.getInstance().getSettingsConfig().getConfig().getStringList("QUEUE_SERVERS")) {
        ...
        }).runTaskTimer(Hub.getInstance(), 30L, 30L);
    }

    public int playerCount() {
        for(QueueData queue : queues) {
            for(String queues : Hub.getInstance().getSettingsConfig().getConfig().getStringList("QUEUE_SERVERS")) {
