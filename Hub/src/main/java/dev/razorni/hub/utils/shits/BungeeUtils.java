...
import dev.razorni.hub.utils.BungeeServer;
...
public class BungeeUtils implements PluginMessageListener {
    ...
    @Getter
    private static final List<String> serversName = new ArrayList<>();
    @Getter
    private static int globalPlayers = 0;

    public static void refreshGlobalCount() {
        ByteArrayDataOutput out = ByteStreams.newDataOutput();
        out.writeUTF("PlayerCount");
        out.writeUTF("ALL");
        Player player = Iterables.getFirst(Hub.getInstance().getServer().getOnlinePlayers(), null);
