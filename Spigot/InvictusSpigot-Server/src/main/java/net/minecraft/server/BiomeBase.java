...
public abstract class BiomeBase {
    ...
    public final float a(BlockPosition blockposition) {
        return aV(blockposition.getX(), blockposition.getY(), blockposition.getZ());
    }

    public final float aV(final int x, final int y, final int z) {
        if (y > 64) {
            float f = (float) (BiomeBase.ae.a(x * 1.0D / 8.0D, z * 1.0D / 8.0D) * 4.0D);
