...
public class BlockDoor extends Block {
    ...
    protected BlockDoor(Material material) {
        super(material);
        this.j(this.blockStateList.getBlockData().set(BlockDoor.FACING, EnumDirection.NORTH).set(BlockDoor.OPEN, false).set(BlockDoor.HINGE, BlockDoor.EnumDoorHinge.LEFT).set(BlockDoor.POWERED, Boolean.FALSE).set(BlockDoor.HALF, BlockDoor.EnumDoorHalf.LOWER));
    }
    ...
    public void doPhysics(World world, BlockPosition blockposition, IBlockData iblockdata, Block block) {
        if (iblockdata.get(BlockDoor.HALF) == BlockDoor.EnumDoorHalf.UPPER) {
            ...
            if (!World.a(world, blockposition.getX(), blockposition.getY() - 1, blockposition.getZ())) {
                world.setAir(blockposition);
                flag = true;
                if (iblockdata2.getBlock() == this) {
    ...
    public boolean canPlace(World world, BlockPosition blockposition) {
        return blockposition.getY() < 255 && World.a(world, blockposition.getX(), blockposition.getY() - 1, blockposition.getZ())
                && super.canPlace(world, blockposition) && super.canPlace(world, blockposition.up());
    }
    ...
    public static int e(IBlockAccess iblockaccess, BlockPosition blockposition) {
        ...
        boolean flag = i(i);
        IBlockData iblockdata1 = iblockaccess.getType(blockposition.getX(), blockposition.getY() - 1, blockposition.getZ());
        int j = iblockdata1.getBlock().toLegacyData(iblockdata1);
        int k = flag ? j : i;
        IBlockData iblockdata2 = iblockaccess.getType(blockposition.getX(), blockposition.getY() + 1, blockposition.getZ());
    ...
    public IBlockData updateState(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition) {
        ...
        if (iblockdata.get(BlockDoor.HALF) == BlockDoor.EnumDoorHalf.LOWER) {
            iblockdata1 = iblockaccess.getType(blockposition.getX(), blockposition.getY() + 1, blockposition.getZ());
            if (iblockdata1.getBlock() == this) {
