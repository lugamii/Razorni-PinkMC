...
public class BlockGrass extends Block implements IBlockFragilePlantElement {
    ...
    public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
        ...
        if (InvictusConfig.decayDirt && world.getLightLevel(x, y + 1, z) < 4 && world.getType(x, y + 1, z).getBlock().p() > 2) {
            // CraftBukkit start
            ...
            // CraftBukkit end
        } else if (InvictusConfig.grassIgnoreLight || world.getLightLevel(x, y + 1, z) >= 9) {
            for (int i = 0; i < Math.min(4, Math.max(20, (int) (4 * 100F / world.growthOdds))); ++i) { // Spigot
                ...
                if (iblockdata1.getBlock() == Blocks.DIRT
                        ...
                        && world.getLightLevel(blockposition1.getX(), blockposition1.getY() + 1, blockposition1.getZ()) >= 4 && block.p() <= 2) {
                    // CraftBukkit start
                    ...
                    BlockSpreadEvent event = new BlockSpreadEvent(blockState.getBlock(), bworld.getBlockAt(x, y, z),
                            blockState);
                    world.getServer().getPluginManager().callEvent(event);
    ...
    public void b(World world, Random random, BlockPosition blockposition, IBlockData iblockdata) {
        ...
        while (i < 128) {
            ...
            while (true) {
                if (j < i / 16) {
                    blockposition2 = blockposition2.a(random.nextInt(3) - 1,
                            (random.nextInt(3) - 1) * random.nextInt(3) / 2, random.nextInt(3) - 1);
                    if (world.getType(blockposition2.getX(), blockposition2.getY() - 1, blockposition2.getZ()).getBlock() == Blocks.GRASS
                            && !world.getType(blockposition2).getBlock().isOccluding()) {
                        ++j;
                        continue;
                    }
                } else if (world.getType(blockposition2).getBlock().material == Material.AIR) {
                    if (random.nextInt(8) == 0) {
                        BlockFlowers.EnumFlowerVarient blockflowers_enumflowervarient = world.getBiome(blockposition2)
                                .a(random, blockposition2);
                        BlockFlowers blockflowers = blockflowers_enumflowervarient.a().a();
                        IBlockData iblockdata1 = blockflowers.getBlockData().set(blockflowers.n(),
                                blockflowers_enumflowervarient);

                        if (blockflowers.f(world, blockposition2, iblockdata1)) {
                            // world.setTypeAndData(blockposition2, iblockdata1, 3); // CraftBukkit
                            CraftEventFactory.handleBlockGrowEvent(world, blockposition2.getX(), blockposition2.getY(),
                                    blockposition2.getZ(), iblockdata1.getBlock(),
                                    iblockdata1.getBlock().toLegacyData(iblockdata1)); // CraftBukkit
                        }
                    } else {
                        IBlockData iblockdata2 = Blocks.TALLGRASS.getBlockData().set(BlockLongGrass.TYPE,
                        ...
                        if (Blocks.TALLGRASS.f(world, blockposition2, iblockdata2)) {
                            ...
                            CraftEventFactory.handleBlockGrowEvent(world, blockposition2.getX(), blockposition2.getY(),
                                    blockposition2.getZ(), iblockdata2.getBlock(),
                                    iblockdata2.getBlock().toLegacyData(iblockdata2)); // CraftBukkit
                        }
                    }
                }
