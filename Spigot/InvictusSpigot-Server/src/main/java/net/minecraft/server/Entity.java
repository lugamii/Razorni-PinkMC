...
public abstract class Entity implements ICommandListener {
    ...
    public AxisAlignedBB boundingBox; // Vortex
    ...
    public void setSize(float f, float f1) {
        if (f != width || f1 != length) {
            ...
            width = f;
            length = f1;
            a(new AxisAlignedBB(boundingBox.a, boundingBox.b, boundingBox.c, boundingBox.a + width, boundingBox.b + length, boundingBox.c + width));
            if (width > f2 && !justCreated) {
                move((f2 - width), 0, (f2 - width));
            }
        }
    }
    ...
    public void setPosition(double d0, double d1, double d2) {
        ...
        locZ = d2;
        float f = width / 2.0F;
        a(new AxisAlignedBB(d0 - f, d1, d2 - f, d0 + f, d1 + length, d2 + f));
    }
    ...
    public void K() {
        ...
        lastPitch = pitch;
        lastYaw = yaw;
        if (world instanceof WorldServer && !(this instanceof EntityProjectile || this instanceof EntityArrow)) {
            if (ak) {
                int i = L();

                if (vehicle == null && al++ >= i) {
        ...
        if (locY < -64.0D || paperNetherCheck()) { // PaperSpigot - Configurable top-of-nether void damage
            O();
        }
    ...
    public boolean c(double d0, double d1, double d2) {
        return this.b(boundingBox.c(d0, d1, d2));
    }

    private boolean b(AxisAlignedBB axisalignedbb) {
        return world.getCubes(this, axisalignedbb).isEmpty() && !this.world.containsLiquid(axisalignedbb);
    }
    ...
    public void loadChunks() {
        ((ChunkProviderServer) world.chunkProvider).getChunkAt(MathHelper.floor(locX) >> 4, MathHelper.floor(locZ) >> 4);
    }
    ...
    public void move(double d0, double d1, double d2) {
        ...
        if (noclip) {
            ...
            if (flag) {
                double d9;
                for (d9 = 0.05D; d0 != 0.0D && world.getCubes(this, getBoundingBox().c(d0, -1.0D, 0.0D)).isEmpty(); d6 = d0) {
                    if (d0 < d9 && d0 >= -d9) {
                        d0 = 0.0D;
                    } else if (d0 > 0.0D) {
                        d0 -= d9;
                ...
                for (; d2 != 0.0D && world.getCubes(this, getBoundingBox().c(0.0D, -1.0D, d2)).isEmpty(); d8 = d2) {
                    if (d2 < d9 && d2 >= -d9) {
                        d2 = 0.0D;
                    } else if (d2 > 0.0D) {
                        d2 -= d9;
                ...
                for (; d0 != 0.0D && d2 != 0.0D && world.getCubes(this, getBoundingBox().c(d0, -1.0D, d2)).isEmpty(); d8 = d2) {
                    if (d0 < d9 && d0 >= -d9) {
                        d0 = 0.0D;
                    } else if (d0 > 0.0D) {
                        d0 -= d9;
            ...
            List<AxisAlignedBB> list = world.getCubes(this, getBoundingBox().a(d0, d1, d2));

            AxisAlignedBB axisalignedbb = getBoundingBox();

            AxisAlignedBB axisalignedbb2;
            for (Iterator<AxisAlignedBB> iterator = list.iterator(); iterator.hasNext(); d1 = axisalignedbb2.b(getBoundingBox(), d1))
                axisalignedbb2 = iterator.next();
            ...
            if (S > 0.0F && flag1 && (d6 != d0 || d8 != d2)) {
                ...
                a(axisalignedbb);
                d1 = S;
                List<AxisAlignedBB> list1 = world.getCubes(this, getBoundingBox().a(d6, d1, d8));
                ...
                for (Iterator<AxisAlignedBB> iterator8 = list1.iterator(); iterator8.hasNext(); d1 = axisalignedbb13.b(getBoundingBox(), d1))
                    axisalignedbb13 = iterator8.next();
                a(getBoundingBox().c(0.0D, d1, 0.0D));
            ...
            if (positionChanged && getBukkitEntity() instanceof Vehicle) {
                ...
                VehicleBlockCollisionEvent event = new VehicleBlockCollisionEvent((Vehicle) getBukkitEntity(), bl);
                world.getServer().getPluginManager().callEvent(event);
            }

            if (s_() && !flag && vehicle == null) {
                double d21 = locX - d3;
                double d22 = locY - d4;
                double d23 = locZ - d5;
                ...
                M = (float) (M + MathHelper.sqrt(d21 * d21 + d23 * d23) * 0.6D);
                N = (float) (N + MathHelper.sqrt(d21 * d21 + d22 * d22 + d23 * d23) * 0.6D);
                if (N > h && block.getMaterial() != Material.AIR) {
                    h = (int) N + 1;
                    if (V()) {
                        float f = MathHelper.sqrt(motX * motX * 0.20000000298023224D + motY * motY + motZ * motZ * 0.20000000298023224D) * 0.35F;
                        if (f > 1.0F)
                            f = 1.0F;
                        makeSound(P(), f, 1.0F + (random.nextFloat() - random.nextFloat()) * 0.4F);
                    }

                    if (InvictusConfig.footstepSounds)
                        a(blockposition.getX(), blockposition.getY(), blockposition.getZ(), block);

                    block.a(world, blockposition, this);
                }
            }

            boolean flag2 = U();
            if (world.e(getBoundingBox().shrink(0.001D, 0.001D, 0.001D))) {
                burn(1.0F);
                if (!flag2) {
                    fireTicks++;
                    if (fireTicks <= 0) {
    ...
    protected void checkBlockCollisions() {
        int x = MathHelper.floor(boundingBox.a + 0.001), y = MathHelper.floor(boundingBox.b + 0.001), z = MathHelper.floor(boundingBox.c + 0.001), x1 = MathHelper.floor(boundingBox.d - 0.001), y1 = MathHelper.floor(boundingBox.e - 0.001), z1 = MathHelper.floor(boundingBox.f - 0.001);
        if (world.areChunksLoadedBetween(x, y, z, x1, y1, z1)) {
            for (int i = x; i <= x1; ++i) {
                for (int j = y; j <= y1; ++j) {
                    for (int k = z; k <= z1; ++k) {
                        IBlockData iblockdata = world.getType(i, j, k);
                        try {
                            iblockdata.getBlock().a(world, new BlockPosition(i, j, k), iblockdata, this);
                        } catch (Throwable throwable) {
    ...
    protected void a(int x, int y, int z, Block block) {
        ...
        if (this.world.getType(x, y + 1, z).getBlock() == Blocks.SNOW_LAYER) {
    ...
    public boolean U() {
        ...
        return inWater || world.isRainingAt(x, y, z) || world.isRainingAt(x, MathHelper.floor(y + length), z);
    }
    ...
    protected void X() {
        ...
        for (i = 0; (float) i < 1.0F + this.width * 20.0F; ++i) {
            ...
            this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX + f2, (f1 + 1.0F), this.locZ + f3, this.motX, this.motY - (double) (this.random.nextFloat() * 0.2F), this.motZ, EnumParticle.EMPTY_ARRAY);
        }

        for (i = 0; (float) i < 1.0F + this.width * 20.0F; ++i) {
            f2 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
            f3 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
            this.world.addParticle(EnumParticle.WATER_SPLASH, this.locX + f2, (f1 + 1.0F), this.locZ + f3, this.motX, this.motY, this.motZ, EnumParticle.EMPTY_ARRAY);
        }

    }

    public void Y() {
        if (InvictusConfig.footstepParticles && isSprinting() && !V()) {
            Z();
        }
    }
    ...
    protected void Z() {
        ...
        if (block.b() != -1) {
            this.world.addParticle(EnumParticle.BLOCK_CRACK, this.locX + (this.random.nextFloat() - 0.5D) * this.width, this.boundingBox.b + 0.1D, this.locZ + (this.random.nextFloat() - 0.5D) * this.width, -this.motX * 4.0D, 1.5D, -this.motZ * 4.0D, Block.getCombinedId(iblockdata));
        }
    }
    ...
    public boolean a(Material material) {
        ...
        int y = MathHelper.floor(d0);
        IBlockData iblockdata = world.getType(MathHelper.floor(locX), y, MathHelper.floor(locZ));
        Block block = iblockdata.getBlock();

        if (block.getMaterial() == material) {
            return d0 < (y + 1) - BlockFluids.b(iblockdata.getBlock().toLegacyData(iblockdata)) - 0.11111111F;
        } else {
            return false;
        }
    }
    ...
    public float c(float f) {
        int x = MathHelper.floor(locX), y = MathHelper.floor(locY + getHeadHeight()), z = MathHelper.floor(locZ);

        return this.world.isLoaded(x, y, z) ? this.world.o(x, y, z) : 0.0F;
    }
    ...
    public void e(NBTTagCompound nbttagcompound) {
        try {
            ...
            nbttagcompound.set("Spawn", this.a(this.spawnX, this.spawnY, this.spawnZ));
    ...
    public void f(NBTTagCompound nbttagcompound) {
        try {
            ...
            NBTTagList nbttaglist3 = nbttagcompound.getList("Spawn", 6);

            this.spawnX = nbttaglist3.d(0);
            this.spawnY = nbttaglist3.d(1);
            this.spawnZ = nbttaglist3.d(2);
    ...
    protected NBTTagList a(float... afloat) {
        ...
        for (float d0 : afloat) {
            nbttaglist.add(new NBTTagFloat(d0));
        }
        return nbttaglist;
    }
        ...
        if (!this.noclip) {
            ...
            for (int i = 0; i < 8; ++i) {
                int j = MathHelper.floor(
                        this.locY + (double) (((float) ((i) % 2) - 0.5F) * 0.1F) + (double) this.getHeadHeight());
                ...
                if (x != k || x != j || x != l) {
                    if (this.world.getType(k, j, l).getBlock().w()) {
                        return true;
                    }
                }
            }
    ...
    public void d(BlockPosition blockposition) {
        if (this.portalCooldown > 0) {
            ...
            if (!blockposition.equals(this.an)) {
                ...
                d1 = Math.abs(MathHelper.c(d1 - (shapedetector_shapedetectorcollection.b().e().c() == EnumDirection.EnumAxisDirection.NEGATIVE ? 1 : 0), d0, d0 - (double) shapedetector_shapedetectorcollection.d()));
    ...
    protected boolean j(double d0, double d1, double d2) {
        ...
        List<AxisAlignedBB> list = this.world.a(this.boundingBox);
    ...
    public void appendEntityCrashDetails(CrashReportSystemDetails crashreportsystemdetails) {
        crashreportsystemdetails.a("Entity Type", () -> EntityTypes.b(Entity.this) + " (" + Entity.this.getClass().getCanonicalName() + ")");
        crashreportsystemdetails.a("Entity ID", this.id);
        crashreportsystemdetails.a("Entity Name", Entity.this::getName);
        ...
        crashreportsystemdetails.a("Entity's Rider", () -> Entity.this.passenger.toString());
        crashreportsystemdetails.a("Entity's Vehicle", () -> Entity.this.vehicle.toString());
    }
    ...
    public EntityTrackerEntry getEntry(EntityTracker entityTracker, int b, int c, boolean flag) {
        return new EntityTrackerEntry(this, b, c, flag);
    }
