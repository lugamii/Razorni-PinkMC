...
public class EntityArmorStand extends EntityLiving {
    ...
    private void A() {
        if (this.world instanceof WorldServer) {
            ((WorldServer) this.world).a(EnumParticle.BLOCK_DUST, this.locX, this.locY + (double) this.length / 1.5D, this.locZ, 10, (this.width / 4.0F), (double) (this.length / 4.0F), (double) (this.width / 4.0F), 0.05D, new int[] { Block.getCombinedId(Blocks.PLANKS.getBlockData())});
        }
    ...
    private void C() {
        Block.a(this.world, MathHelper.floor(locX), MathHelper.floor(locY), MathHelper.floor(locZ), new ItemStack(Items.ARMOR_STAND));
        this.D();
    }
        ...
        for (int i = 0; i < this.items.length; ++i) {
            if (this.items[i] != null && this.items[i].count > 0) {
                if (this.items[i] != null) {
                    Block.a(this.world, MathHelper.floor(locX), MathHelper.floor(locY) + 1, MathHelper.floor(locZ), this.items[i]);
                }
