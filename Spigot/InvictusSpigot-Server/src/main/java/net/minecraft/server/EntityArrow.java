...
public class EntityArrow extends Entity implements IProjectile {
    ...
    public void t_() {
        ...
        if (this.lastPitch == 0.0F && this.lastYaw == 0.0F) {
            ...
            this.lastPitch = this.pitch = (float) (MathHelper.b(this.motY, MathHelper.sqrt(this.motX * this.motX + this.motZ * this.motZ)) * 180.0D / 3.1415927410125732D);
        }

        IBlockData iblockdata = this.world.getType(d, e, f);
        ...
        if (this.inGround) {
            ...
            for (Entity entity1 : world.getEntities(this, this.boundingBox.a(this.motX, this.motY, this.motZ).grow(1.0D, 1.0D, 1.0D))) {
                if (entity1.ad() && (entity1 != this.shooter || this.as >= 5)) {
                    AxisAlignedBB axisalignedbb1 = entity1.boundingBox.grow(0.3, 0.3, 0.3);
            ...
            if (movingobjectposition != null && entity instanceof EntityPlayer && shooter != null) {
                EntityHuman entityhuman = (EntityHuman) entity;
                if (entityhuman.abilities.isInvulnerable || this.shooter instanceof EntityPlayer && !((EntityHuman) this.shooter).a(entityhuman)) {
                    movingobjectposition = null;
                    entity = null;
                } else if (shooter instanceof EntityPlayer && !((EntityPlayer) shooter).getBukkitEntity().canSee(((EntityPlayer) entity).getBukkitEntity())) {
                    movingobjectposition = null;
                    entity = null;
                }
            }
            ...
            if (movingobjectposition != null) {
                ...
                if (entity != null) {
                    ...
                    if (entity.damageEntity(damagesource, k)) {
                        if (this.isBurning() && !(entity instanceof EntityEnderman) && (!(entity instanceof EntityPlayer) || !(shooter instanceof EntityPlayer) || this.world.pvpMode)) { // CraftBukkit - abide by pvp setting if destination is a player
                            ...
                            if (this.knockbackStrength > 0) {
                                ...
                                if (f3 > 0.0F) {
                                    if (movingobjectposition.entity instanceof EntityPlayer) {
                                        EntityPlayer player = (EntityPlayer) movingobjectposition.entity;
                                        KnockbackProfile profile = player.getKnockbackProfile();
                                        Vector unitVector = new Vector(motX, motY, motZ).normalize();
                                        double horizontal = profile.getSettingValue("bowHozirontal");
                                        double velX = unitVector.getX() / 1.6D * horizontal;
                                        double velZ = unitVector.getZ() / 1.6D * horizontal;
                                        if (knockbackStrength > 0) {
                                            double modifier = knockbackStrength * (Double) profile.getSettingValue("bowPunchMultiplier") + 1;
                                            velX *= modifier;
                                            velZ *= modifier;
                                        }
                                        entity.motX = velX;
                                        entity.motY = 0.36D * (Double) profile.getSettingValue("bowVertical");
                                        entity.motZ = velZ;
                                    } else movingobjectposition.entity.g(this.motX * this.knockbackStrength * 0.6000000238418579D / f3, 0.1D, this.motZ * this.knockbackStrength * 0.6000000238418579D / f3);
                                }
                            }
                            if (this.shooter != null && shooter instanceof EntityLiving) {
                    ...
                    IBlockData iblockdata1 = this.world.getType(d, e, f);
                    ...
                    double f1 = MathHelper.sqrt(this.motX * this.motX + this.motY * this.motY + this.motZ * this.motZ);
            ...
            if (this.isCritical()) {
                for (j = 0; j < 4; ++j) {
                    this.world.addParticle(EnumParticle.CRIT, this.locX + this.motX * j / 4.0D, this.locY + this.motY * j / 4.0D, this.locZ + this.motZ * j / 4.0D, -this.motX, -this.motY + 0.2D, -this.motZ, EnumParticle.EMPTY_ARRAY);
                }
            }
            ...
            if (this.V()) {
                for (int l = 0; l < 4; ++l)
                    this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX - this.motX * 0.25F, this.locY - this.motY * 0.25F, this.locZ - this.motZ * 0.25F, this.motX, this.motY, this.motZ, EnumParticle.EMPTY_ARRAY);
                f4 = 0.6F;
            }
