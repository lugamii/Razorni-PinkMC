...
public class EntityEnderDragon extends EntityInsentient implements IComplex, IMonster {
    ...
    public void m() {
        ...
        if (this.getHealth() <= 0.0F) {
            ...
            this.world.addParticle(EnumParticle.EXPLOSION_LARGE, this.locX + (double) f, this.locY + 2.0D + (double) f1, this.locZ + (double) f2, 0.0D, 0.0D, 0.0D, EnumParticle.EMPTY_ARRAY);
        } else {
            this.n();
            ...
            if (this.ce()) {
                ...
                float f3;
                    ...
                    if (this.target != null) {
                        ...
                        this.b = this.target.boundingBox.b + d4;
                    } else {
                ...
                if (this.hurtTicks == 0) {
                    this.a(this.world.getEntities(this, this.bs.boundingBox.grow(4.0D, 2.0D, 4.0D).c(0.0D, -2.0D, 0.0D)));
                    this.a(this.world.getEntities(this, this.bt.boundingBox.grow(4.0D, 2.0D, 4.0D).c(0.0D, -2.0D, 0.0D)));
                    this.b(this.world.getEntities(this, this.bn.boundingBox.grow(1.0D, 1.0D, 1.0D)));
                }
                ...
                for (int j = 0; j < 3; ++j) {
                    ...
                    entitycomplexpart.setPositionRotation(this.locX - (double) ((f11 * f17 + f15 * f18) * f2), this.locY + (adouble2[1] - adouble[1]) - (double) ((f18 + f17) * f9) + 1.5D, this.locZ + (double) ((f12 * f17 + f16 * f18) * f2), 0.0F, 0.0F);
                }

                    this.bx = this.b(this.bn.boundingBox) | this.b(this.bo.boundingBox);
    ...
    private void n() {
        ...
        if (this.random.nextInt(10) == 0) {
            float f = 32.0F;
            List<EntityEnderCrystal> list = this.world.a(EntityEnderCrystal.class, this.boundingBox.grow(f, f, f));
    ...
    private void a(List<Entity> list) {
        double d0 = (this.bo.boundingBox.a + this.bo.boundingBox.d) / 2.0D;
        double d1 = (this.bo.boundingBox.c + this.bo.boundingBox.f) / 2.0D;
    ...
    private void cf() {
        ...
        arraylist.removeIf(EntityHuman::isSpectator);
    ...
    private boolean b(AxisAlignedBB axisalignedbb) {
        ...
        for (int k1 = i; k1 <= l; ++k1) {
            for (int l1 = j; l1 <= i1; ++l1) {
                for (int i2 = k; i2 <= j1; ++i2) {
                    Block block = this.world.getType(k1, l1, i2).getBlock();
        ...
        if (flag1) {
            ...
            if (event.isCancelled()) {
                ...
                for (org.bukkit.block.Block block : event.blockList()) {
                    ...
                    if (nmsBlock.a(explosionSource)) {
                        nmsBlock.dropNaturally(this.world, pos, nmsBlock.fromLegacyData(block.getData()), event.getYield(), 0);
                    }

                    nmsBlock.wasExploded(world, pos, explosionSource);

                    this.world.setAir(pos);
                }
            }
            // CraftBukkit end
            ...
            this.world.addParticle(EnumParticle.EXPLOSION_LARGE, d0, d1, d2, 0.0D, 0.0D, 0.0D, EnumParticle.EMPTY_ARRAY);
        }
    ...
    protected void aZ() {
        ...
        if (this.by >= 180 && this.by <= 200) {
            ...
            this.world.addParticle(EnumParticle.EXPLOSION_HUGE, this.locX + (double) f, this.locY + 2.0D + (double) f1, this.locZ + (double) f2, 0.0D, 0.0D, 0.0D, EnumParticle.EMPTY_ARRAY);
        }
            ...
            if (this.by == 1) {
                ...
                for (EntityPlayer player : MinecraftServer.getServer().getPlayerList().players) {
                    ...
                    if (distanceSquared > viewDistance * viewDistance) {
                        ...
                        player.playerConnection.sendPacket(new PacketPlayOutWorldEvent(1018, new BlockPosition(relativeX, this.locY, relativeZ), 0, true));
                    } else {
                        player.playerConnection.sendPacket(new PacketPlayOutWorldEvent(1018, new BlockPosition(this.locX, this.locY, this.locZ), 0, true));
                    }
                }
                // CraftBukkit end
            }
