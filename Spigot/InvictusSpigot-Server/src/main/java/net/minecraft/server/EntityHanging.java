...
public abstract class EntityHanging extends Entity {
    ...
    public static AxisAlignedBB calculateBoundingBox(BlockPosition blockPosition, EnumDirection direction, int width, int height) {
            ...
            d0 -= (double) direction.getAdjacentX() * 0.111146875D;
            d2 -= (double) direction.getAdjacentZ() * 0.111146875D;
            d1 += d5;
    ...
    public void t_() {
        ...
        if (this.c++ == this.world.spigotConfig.hangingTickFrequency) {
            this.c = 0;
            if (!this.dead && !this.survives()) {
                // CraftBukkit start - fire break events
                Material material = this.world.getType(MathHelper.floor(locX), MathHelper.floor(locY),MathHelper.floor(locZ)).getBlock().getMaterial();
    ...
    public void g(double d0, double d1, double d2) {
        if (!this.dead && d0 * d0 + d1 * d1 + d2 * d2 > 0.0D) { // CraftBukkit - not needed
            this.die();
            this.b((Entity) null);
        }
