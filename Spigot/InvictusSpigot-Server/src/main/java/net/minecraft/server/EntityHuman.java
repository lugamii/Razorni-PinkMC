...
public abstract class EntityHuman extends EntityLiving {
    ...
    private int h;
    private int i;
    private KnockbackProfile knockbackProfile = KnockbackManager.DEFAULT_KNOCKBACK;

    public EntityHuman(World world, GameProfile gameprofile) {
        super(world);
        this.uniqueID = a(gameprofile);
        this.bH = gameprofile;
        this.defaultContainer = new ContainerPlayer(this.inventory, true, this);
    ...
    private static BlockPosition getBed0(World world, BlockPosition blockposition, boolean flag) {
        ...
        if (block != Blocks.BED) {
            if (!flag) {
                ...
                return block.g() && world.getType(blockposition.getX(), blockposition.getY() + 1, blockposition.getZ()).getBlock().g() ? blockposition : null;
            }
        } else {
    ...
    @Override
    public EntityTrackerEntry getEntry(EntityTracker entityTracker, int i, int j, boolean flag) {
        return new PlayerTrackerEntry(entityTracker, this, i, j, flag);
    }
    ...
    public boolean isAnimating() {
        return this.bS() && this.g.getItem().e(this.g) != EnumAnimation.NONE;
    }

    public boolean isEating() {
        return this.bS() && this.g.getItem().e(this.g) == EnumAnimation.EAT;
    }
    ...
    protected void b(ItemStack itemstack, int i) {
        switch (itemstack.m()) { // Vortex - use switch case
            case DRINK: {
            ...
            case EAT: {
                for (int j = 0; j < i; ++j) {
                    ...
                    if (itemstack.usesData()) {
                        world.addParticle(EnumParticle.ITEM_CRACK, vec3d1.a, vec3d1.b, vec3d1.c, vec3d.a, vec3d.b + 0.05D, vec3d.c, Item.getId(itemstack.getItem()), itemstack.getData());
                    } else {
                        world.addParticle(EnumParticle.ITEM_CRACK, vec3d1.a, vec3d1.b, vec3d1.c, vec3d.a, vec3d.b + 0.05D, vec3d.c, Item.getId(itemstack.getItem()));
                    }
                }
    ...
    protected void s() {
        if (this.g != null) {
            ...
            if (event.isCancelled()) {
                // Update client
                EntityPlayer pl = (EntityPlayer) this;
                pl.playerConnection.sendPacket(new PacketPlayOutSetSlot((byte) 0,
                        activeContainer.getSlot(this.inventory, this.inventory.itemInHandIndex).index,
                        this.g));
                // Spigot Start
                pl.getBukkitEntity().updateInventory();
                pl.getBukkitEntity().updateScaledHealth();
                // Spigot End
                return;
            }
    ...
    public void m() {
        ...
        if (this.getHealth() > 0.0F && !this.isSpectator()) {
            ...
            if (this.ae()) { // Spigot: Add this.ae() condition (second !this.isDead near bottom of
                for (Entity entity : world.getEntities(this, axisalignedbb)) {
                    if (!entity.dead) {
                        this.d(entity);
                    }
                }
    ...
    public void addScore(int i) {
        this.datawatcher.watch(18, getScore() + i);
    }
    ...
    public void die(DamageSource damagesource) {
        ...
        /* - Vortex less useless statistic increments
    ...
    public void b(Entity entity, int i) {
        ...
        Collection<ScoreboardScore> collection = this.world.getServer().getScoreboardManager()
                .getScoreboardScores(IScoreboardCriteria.f, this.getName(), new java.util.ArrayList<>());
    ...
    public EntityItem a(boolean flag) {
        ...
        ItemStack hand = inventory.getItemInHand(); // Vortex - 1 time getting item in hand
        return this.a(
                this.inventory.splitStack(this.inventory.itemInHandIndex,
                        flag && hand != null ? hand.count : 1),
                false, true);
    }
    ...
    public EntityItem a(ItemStack itemstack, boolean flag, boolean flag1) {
        if (itemstack == null) {
            ...
            if (flag) {
                ...
                entityitem.motX += FastMath.cos(f1) * (double) f;
                entityitem.motY += ((this.random.nextFloat() - this.random.nextFloat()) * 0.1F);
                entityitem.motZ += FastMath.sin(f1) * (double) f;
            }
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        if (isInvulnerable(damagesource) || (this.abilities.isInvulnerable && !damagesource.ignoresInvulnerability()))
            return false;
        ...
        if (entity.aD()) {
            if (!entity.l(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    double victimMotX = entity.motX, victimMotY = entity.motY, victimMotZ = entity.motZ;

                    if (entity.damageEntity(DamageSource.playerAttack(this), f)) {
                        KnockbackProfile profile = entity instanceof EntityPlayer ? ((EntityPlayer) entity).getKnockbackProfile() : getKnockbackProfile();
                        double velX, velY = profile.getSettingValue("vertical"), velZ;

                        boolean onePointSeven = (Boolean) profile.getSettingValue("17kb") && getBukkitEntity().getLocation().getDirection().dot(entity.getBukkitEntity().getLocation().getDirection()) > 0.1;
                        double horizontal = onePointSeven ? profile.getSettingValue("17kbHorizontal") : profile.getSettingValue("horizontal");

                        if ((Boolean) profile.getSettingValue("inheritHorizontal")) {
                            double friction = profile.getSettingValue("frictionHorizontal");
                            velX = (entity.motX * friction) + FastMath.sin(Math.toRadians(yaw)) * -1.0F;
                            velZ = (entity.motZ * friction) + FastMath.cos(Math.toRadians(yaw));
                        } else {
                            velX = FastMath.sin(Math.toRadians(aK)) * -1.0D;
                            velZ = FastMath.cos(Math.toRadians(aK));
                        }

                        velX *= horizontal;
                        velZ *= horizontal;

                        if (!entity.onGround && !onePointSeven) {
                            double groundHorizontal = profile.getSettingValue("horizontalOnGround");
                            velX *= groundHorizontal;
                            velY *= (Double) profile.getSettingValue("verticalOnGround");
                            velZ *= groundHorizontal;
                        }

                        int level = EnchantmentManager.getEnchantmentLevel(Enchantment.KNOCKBACK.id, inventory.getItemInHand());
                        if (level > 0) {
                            level++;
                            velX *= level - 0.3D;
                            velZ *= level - 0.3D;
                        ...
                        if (shouldDealSprintKnockback && !onePointSeven) {
                            double horizontalSprint = profile.getSettingValue("horizontalSprinting");
                            velX *= horizontalSprint;
                            velY *= (Double) profile.getSettingValue("verticalSprinting");
                            velZ *= horizontalSprint;
                            shouldDealSprintKnockback = false;
                        }

                        if (isSprinting()) {
                            double attackerSlowndown = profile.getSettingValue("slowdown");
                            motX *= attackerSlowndown;
                            motZ *= attackerSlowndown;

                            shouldDealSprintKnockback = false;
                            if ((Boolean) profile.getSettingValue("cancelSprint"))
                                setSprinting(false);
                        }

                        double yOff = entity.locY - locY;
                        if ((Boolean) profile.getSettingValue("comboMode")) {
                            double height = profile.getSettingValue("comboHeight");
                            if (yOff > height)
                                ticksDown = MinecraftServer.currentTick;
                            if (yOff > height || MinecraftServer.currentTick - ticksDown < (Integer) profile.getSettingValue("comboTicks"))
                                velY = profile.getSettingValue("comboVelocity");
                        }

                        if (entity instanceof EntityPlayer && entity.velocityChanged) {
                            ((EntityPlayer) entity).getBukkitEntity().setVelocity(new Vector(velX, velY, velZ));
    ...
    public float bI() {
        if (this.isSprinting()) { // if player is not sprinting (Which probably means they're stationary) dont
            // check the hashmap
            return (float) this.getAttributeInstance(GenericAttributes.MOVEMENT_SPEED).getValue();
        } else {
            return (float) ((double) this.abilities.b());
        }
    }
    ...
    public void checkMovement(double d0, double d1, double d2) {
        if (this.vehicle == null) {
            ...
            if (this.a(Material.WATER)) {
                ...
                if (i > 0) {
                    this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * (float) i * 0.01F);
                }
            } else if (this.V()) {
    ...
    public boolean a(BlockPosition blockposition, EnumDirection enumdirection, ItemStack itemstack) {
        if (this.abilities.mayBuild) {
            ...
            return itemstack.d(world.getType(blockposition.getX() + enumdirection.getAdjacentX(), blockposition.getY() + enumdirection.getAdjacentY(), blockposition.getZ() + enumdirection.getAdjacentZ()).getBlock())
                    || itemstack.x();
        }
    }
    ...
    protected int getExpValue(EntityHuman entityhuman) {
        if (this.world.getGameRules().getBoolean("keepInventory")) {
            ...
            return Math.min(expLevel * 7, 100);
        }
    }
    ...
    public KnockbackProfile getKnockbackProfile() {
        return knockbackProfile == null ? KnockbackManager.DEFAULT_KNOCKBACK : knockbackProfile;
    }

    public void setKnockbackProfile(KnockbackProfile profile) {
        knockbackProfile = profile == null ? KnockbackManager.DEFAULT_KNOCKBACK : profile;
    }
    ...
    public enum EnumChatVisibility {
        ...
        static {
            for (EnumChatVisibility entityhuman_enumchatvisibility : values()) {
