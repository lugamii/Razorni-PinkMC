...
public class EntityIronGolem extends EntityGolem {
    ...
    protected void E() {
        if (--this.b <= 0) {
            ...
            this.a = this.world.ae().getClosestVillage(MathHelper.floor(locX), MathHelper.floor(locY), MathHelper.floor(locZ), 32);
            if (this.a == null) {
                this.cj();
            } else {
                this.a(a.a(), (int) ((float) this.a.b() * 0.6F));
            }
        }
    ...
    public void m() {
        ...
        if (this.motX * this.motX + this.motZ * this.motZ > 2.500000277905201E-7D && this.random.nextInt(5) == 0) {
            ...
            if (block.getMaterial() != Material.AIR) {
                this.world.addParticle(EnumParticle.BLOCK_CRACK, this.locX + (this.random.nextFloat() - 0.5D) * this.width, this.getBoundingBox().b + 0.1D, this.locZ + (this.random.nextFloat() - 0.5D) * this.width, 4.0D * (this.random.nextFloat() - 0.5D), 0.5D, (this.random.nextFloat() - 0.5D) * 4.0D, Block.getCombinedId(iblockdata));
            }
        }
