...
public class EntityItem extends Entity implements HopperPusher {
    ...
    public void t_() {
        ...
        if (itemStack == null) {
            ...
            super.t_();
            if (ticksLived % 15 == 0 && tryPutInHopper())  // Vortex - optimized ticking
                return;
            ...
            this.noclip = this.j(this.locX, (boundingBox.b + boundingBox.e) / 2.0D, this.locZ);
            this.move(this.motX, this.motY, this.motZ);

            // Vortex - optimized ticking while entity doesnt change location
            if ((int) this.lastX != (int) this.locX || (int) this.lastY != (int) this.locY || (int) this.lastZ != (int) this.locZ || this.ticksLived % 25 == 0) {
                if (world.getType(MathHelper.floor(locX), MathHelper.floor(locY), MathHelper.floor(locZ)).getBlock().getMaterial() == Material.LAVA) {
                    this.motY = 0.20000000298023224D;
                    this.motX = ((this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
                    this.motZ = ((this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
                    if (InvictusConfig.itemFizzSounds)
            ...
            if (this.onGround) {
                f = this.world.getType(MathHelper.floor(this.locX), MathHelper.floor(boundingBox.b) - 1, MathHelper.floor(this.locZ)).getBlock().frictionFactor * 0.98F;
            }
    ...
    public void inactiveTick() {
        if (ticksLived % 15 == 0 && tryPutInHopper()) // Vortex - optimized inactive ticking
            return;
        // CraftBukkit start - Use wall time for pickup and despawn timers
        int elapsedTicks = MinecraftServer.currentTick - this.lastTick;
        if (this.pickupDelay != 32767) this.pickupDelay -= elapsedTicks;
    ...
    private void w(ItemStack itemStack) {
        if (itemStack.count >= itemStack.getMaxStackSize()) { //Vortex: dont merge items
            return;
        }
        // Spigot start
        ...
        for (EntityItem entityItem : this.world.a(EntityItem.class, boundingBox.grow(radius, radius, radius))) {
            this.a(entityItem);
        }
    ...
    public boolean W() {
        this.inWater = this.world.a(boundingBox, Material.WATER, this);
        return this.inWater;
    }
    ...
    public void d(EntityHuman entityhuman) {
        if (!((EntityPlayer)entityhuman).getBukkitEntity().canSeeEntity(getBukkitEntity()))
            return;
        ItemStack itemstack = this.getItemStack();
