...
public class EntitySlime extends EntityInsentient implements IMonster {
    ...
    public void t_() {
        ...
        if (this.onGround && !this.bk) {
            ...
            for (int j = 0; j < i * 8; ++j) {
                ...
                world.addParticle(enumparticle, d0, this.getBoundingBox().b, d1, 0.0D, 0.0D, 0.0D, EnumParticle.EMPTY_ARRAY);
            }
    ...
    public boolean bR() {
        int x = MathHelper.floor(this.locX), z = MathHelper.floor(locZ);
        Chunk chunk = this.world.getChunkAtWorldCoords(x, z);
        if (this.world.getWorldData().getType() != WorldType.FLAT || this.random.nextInt(4) == 1) {
            if (this.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
                BiomeBase biomebase = this.world.getBiome(x, 0, z);
