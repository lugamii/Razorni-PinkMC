...
public class EntityZombie extends EntityMonster {
    ...
    public void m() {
        if (this.world.w() && !this.isBaby()) {
            float f = this.c(1.0F);
            if (f > 0.5F && this.random.nextFloat() * 30.0F < (f - 0.4F) * 2.0F && this.world.i(MathHelper.floor(this.locX), MathHelper.floor(FastMath.round(this.locY)), MathHelper.floor(this.locZ))) {
                boolean flag = true;
            ...
            if (entityliving != null && this.world.getDifficulty() == EnumDifficulty.HARD && (double) this.random.nextFloat() < this.getAttributeInstance(EntityZombie.a).getValue()) {
                ...
                for (int l = 0; l < 50; ++l) {
                    ...
                    if (World.a(this.world, i1, j1 - 1, k1) && this.world.getLightLevel(i1, j1, k1) < 10) {
                        entityzombie.setPosition(i1, 1, k1);
                        if (!this.world.isPlayerNearby(i1, j1, k1, 7.0D) && this.world.a(entityzombie.getBoundingBox(), entityzombie) && this.world.getCubes(entityzombie, entityzombie.getBoundingBox()).isEmpty() && !this.world.containsLiquid(entityzombie.getBoundingBox())) {
                            ...
                            entityzombie.prepare(this.world.E(MathHelper.floor(entityzombie.locX), MathHelper.floor(entityzombie.locY), MathHelper.floor(entityzombie.locZ)), null);
        ...
        if (object instanceof GroupDataZombie) {
            ...
            if (entityzombie_groupdatazombie.a) {
                ...
                if ((double) this.world.random.nextFloat() < 0.05D) {
                    List<EntityChicken> list = this.world.a(EntityChicken.class, this.getBoundingBox().grow(5.0D, 3.0D, 5.0D), IEntitySelector.b);
    ...
    protected void cq() {
        ...
        entityvillager.prepare(this.world.E(MathHelper.floor(entityvillager.locX), MathHelper.floor(entityvillager.locY),MathHelper.floor(entityvillager.locZ)), null);
        entityvillager.cp();
        if (this.isBaby()) {
            ...
            for (int k = (int) this.locX - 4; k < (int) this.locX + 4 && j < 14; ++k) {
                for (int l = (int) this.locY - 4; l < (int) this.locY + 4 && j < 14; ++l) {
                    for (int i1 = (int) this.locZ - 4; i1 < (int) this.locZ + 4 && j < 14; ++i1) {
                        Block block = this.world.getType(k, l, i1).getBlock();
    ...
    static class GroupDataZombie implements GroupDataEntity {
        ...
        GroupDataZombie(boolean flag, boolean flag1, SyntheticClass_1 entityzombie_syntheticclass_1) {
            this(flag, flag1);
        }
    }
}
