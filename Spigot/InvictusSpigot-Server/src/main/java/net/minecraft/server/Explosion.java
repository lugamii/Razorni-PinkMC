...
public class Explosion {
    ...
    public void a(boolean flag) {
        ...
        if (InvictusConfig.explosionSounds)
            world.makeSound(posX, posY, posZ, "random.explode", source instanceof EntityTNTPrimed ? world.paperSpigotConfig.tntExplosionVolume : 4.0f, (1.0f + (world.random.nextFloat() - world.random.nextFloat()) * 0.2f) * 0.7f);
        if (InvictusConfig.explosionParticles) {
            if (size >= 2.0f && b) {
                world.addParticle(EnumParticle.EXPLOSION_HUGE, posX, posY, posZ, 1.0, 0.0, 0.0, EnumParticle.EMPTY_ARRAY);
            } else {
                world.addParticle(EnumParticle.EXPLOSION_LARGE, posX, posY, posZ, 1.0, 0.0, 0.0, EnumParticle.EMPTY_ARRAY);
            }
        }
        ...
        if (this.b) {
            ...
            while (iterator.hasNext()) {
                ...
                if (flag) {
                    ...
                    d4 *= d7;
                    d5 *= d7;
                    this.world.addParticle(EnumParticle.EXPLOSION_NORMAL, (d0 + this.posX) / 2.0D, (d1 + this.posY) / 2.0D, (d2 + this.posZ) / 2.0D, d3, d4, d5, EnumParticle.EMPTY_ARRAY);
                    this.world.addParticle(EnumParticle.SMOKE_NORMAL, d0, d1, d2, d3, d4, d5, EnumParticle.EMPTY_ARRAY);
                }
