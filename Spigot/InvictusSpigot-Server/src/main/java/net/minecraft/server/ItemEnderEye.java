...
public class ItemEnderEye extends Item {
    ...
    public boolean interactWith(ItemStack itemstack, EntityHuman entityhuman, World world, BlockPosition blockposition, EnumDirection enumdirection, float f, float f1, float f2) {
        ...
        if (entityhuman.a(blockposition.shift(enumdirection), enumdirection, itemstack) && iblockdata.getBlock() == Blocks.END_PORTAL_FRAME && !iblockdata.get(BlockEnderPortalFrame.EYE)) {
            world.setTypeAndData(blockposition, iblockdata.set(BlockEnderPortalFrame.EYE, Boolean.TRUE), 2);
            world.updateAdjacentComparators(blockposition, Blocks.END_PORTAL_FRAME);
            --itemstack.count;

            for (int i = 0; i < 16; ++i) {
                world.addParticle(EnumParticle.SMOKE_NORMAL, blockposition.getX() + (5.0F + ItemEnderEye.g.nextFloat() * 6.0F) / 16.0F, blockposition.getY() + 0.8125F, blockposition.getZ() + (5.0F + ItemEnderEye.g.nextFloat() * 6.0F) / 16.0F, 0, 0, 0, EnumParticle.EMPTY_ARRAY);
            }
            ...
            for (int l = -2; l <= 2; ++l) {
                ...
                if (iblockdata1.getBlock() == Blocks.END_PORTAL_FRAME) {
                    if (!iblockdata1.get(BlockEnderPortalFrame.EYE)) {
                        flag1 = false;
                        break;
                    }
            ...
            if (flag1 && k == j + 2) {
                ...
                for (i1 = j; i1 <= k; ++i1) {
                    ...
                    if (iblockdata2.getBlock() != Blocks.END_PORTAL_FRAME || !iblockdata2.get(BlockEnderPortalFrame.EYE)) {
                        flag1 = false;
                        break;
                    }
                }
                ...
                for (i1 = j - 1; i1 <= k + 1; i1 += 4) {
                    ...
                    for (j1 = 1; j1 <= 3; ++j1) {
                        ...
                        if (iblockdata3.getBlock() != Blocks.END_PORTAL_FRAME || !iblockdata3.get(BlockEnderPortalFrame.EYE)) {
                            flag1 = false;
                            break;
                        }
                    }
    ...
    public ItemStack a(ItemStack itemstack, World world, EntityHuman entityhuman) {
        ...
        if (movingobjectposition == null || movingobjectposition.type != MovingObjectPosition.EnumMovingObjectType.BLOCK || world.getType(movingobjectposition.a()).getBlock() != Blocks.END_PORTAL_FRAME) {
            ...
            if (blockposition != null) {
                ...
                world.a(null, 1002, new BlockPosition(entityhuman), 0);
