...
public class MobEffectList {
    ...
    public void tick(EntityLiving entityliving, int i) {
        final boolean heal = this.id == MobEffectList.HEAL.id, harm = id == MobEffectList.HARM.id, undead = entityliving.bm();
        ...
        } else if (this.id == MobEffectList.HUNGER.id && entityliving instanceof EntityPlayer) {
            ((EntityHuman) entityliving).applyExhaustion(0.025F * (float) (i + 1));
        } else if (this.id == MobEffectList.SATURATION.id && entityliving instanceof EntityPlayer) {
        ...
        } else if (harm && !undead || heal && undead) {
    ...
    public void applyInstantEffect(Entity entity, Entity entity1, EntityLiving entityliving, int i, double d0) {
        int j;
        final boolean heal = this.id == MobEffectList.HEAL.id, undead = entityliving.bm();
        if ((!heal || undead) && (this.id != MobEffectList.HARM.id || !undead)) {
            if (this.id == MobEffectList.HARM.id || heal) {
                j = (int) (d0 * (6 << i) + 0.5D);
                if (entity == null) {
    ...
    public MobEffectList a(IAttribute iattribute, String s, double d0, int i) {
        this.J.put(iattribute, new AttributeModifier(FastUUID.parseUUID(s), this.a(), d0, i));
        return this;
    }
    ...
    public void a(EntityLiving entityliving, AttributeMapBase attributemapbase, int i) {
        ...
        for (Entry<IAttribute, AttributeModifier> iAttributeAttributeModifierEntry : this.J.entrySet()) {
            AttributeInstance attributeinstance = attributemapbase.a(iAttributeAttributeModifierEntry.getKey());

            if (attributeinstance != null) {
                attributeinstance.c(iAttributeAttributeModifierEntry.getValue());
            }
        }
    ...
    public void b(EntityLiving entityliving, AttributeMapBase attributemapbase, int i) {
        ...
        for (Entry<IAttribute, AttributeModifier> iAttributeAttributeModifierEntry : this.J.entrySet()) {
            ...
            if (attributeinstance != null) {
                AttributeModifier attributemodifier = iAttributeAttributeModifierEntry.getValue();
