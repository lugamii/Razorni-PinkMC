...
public class WorldGenAcaciaTree extends WorldGenTreeAbstract {
    ...
    private static final IBlockData b = Blocks.LEAVES2.getBlockData().set(BlockLeaves2.VARIANT, BlockWood.EnumLogVariant.ACACIA).set(BlockLeaves.CHECK_DECAY, Boolean.FALSE);
            ...
            if (!flag) {
                ...
                if ((block == Blocks.GRASS || block == Blocks.DIRT) && blockposition.getY() < 256 - i - 1) {
                    ...
                    for (int i2 = 0; i2 < i; ++i2) {
                        ...
                        Material material = world.getType(k, l1, j1).getBlock().getMaterial();

                        if (material == Material.AIR || material == Material.LEAVES) {
                            this.b(world, new BlockPosition(k, l1, j1));
                            k1 = l1;
                        }
                    }
                    ...
                    for (l1 = -3; l1 <= 3; ++l1) {
                        for (j2 = -3; j2 <= 3; ++j2) {
                            if (FastMath.abs(l1) != 3 || FastMath.abs(j2) != 3) {
                    ...
                    if (enumdirection1 != enumdirection) {
                        ...
                        for (int l2 = l1; l2 < i && j2 > 0; --j2) {
                            if (l2 >= 1) {
                                ...
                                Material material1 = world.getType(k, k2, j1).getBlock().getMaterial();

                                if (material1 == Material.AIR || material1 == Material.LEAVES) {
                                    this.b(world, new BlockPosition(k, k2, j1));
                                    k1 = k2;
                                }
                            }
