...
public class WorldGenBigTree extends WorldGenTreeAbstract {
    ...
    void a() {
        ...
        int i = (int) (1.382D + FastMath.pow(this.f * (double) this.a / 13.0D, 2.0D));
        ...
        this.j.add(new Position(this.m.up(k), j));
        ...
        for (; k >= 0; --k) {
            ...
            if (f >= 0.0F) {
                for (int l = 0; l < i; ++l) {
                    ...
                    double d2 = d0 * FastMath.sin(d1) + 0.5D;
                    double d3 = d0 * FastMath.cos(d1) + 0.5D;
                    ...
                    if (this.a(blockposition, blockposition1) == -1) {
                        ...
                        double d4 = (double) blockposition.getY() - Math.sqrt(i1 * i1 + j1 * j1) * this.d;
                        int k1 = d4 > (double) j ? j : (int) d4;
                        BlockPosition blockposition2 = new BlockPosition(this.m.getX(), k1, this.m.getZ());

                        if (this.a(blockposition2, blockposition) == -1) {
                            this.j.add(new Position(blockposition, blockposition2.getY()));
                        }
                    }
                }
            }
    ...
    void a(BlockPosition blockposition, float f, IBlockData iblockdata) {
        ...
        for (int j = -i; j <= i; ++j) {
            for (int k = -i; k <= i; ++k) {
                if (FastMath.pow((double) Math.abs(j) + 0.5D, 2.0D) + FastMath.pow((double) Math.abs(k) + 0.5D, 2.0D) <= (double) (f * f)) {
    ...
    void a(BlockPosition blockposition) {
        for (int i = 0; i < this.i; ++i) {
            this.a(blockposition.up(i), this.b(i), Blocks.LEAVES.getBlockData().set(BlockLeaves.CHECK_DECAY, Boolean.FALSE));
        }
    ...
    void a(BlockPosition blockposition, BlockPosition blockposition1, Block block) {
        ...
        for (int j = 0; j <= i; ++j) {
            BlockPosition blockposition3 = blockposition.a(0.5F + (float) j * f, 0.5F + (float) j * f1, 0.5F + (float) j * f2);
    ...
    void b() {
        ...
        for (Position worldgenbigtree_position : this.j) {
    ...
    private boolean f() {
        Block block = this.l.getType(m.getX(), m.getY() - 1, m.getZ()).getBlock();
