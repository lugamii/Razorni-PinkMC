        ...
        if (blockposition.getY() >= 1 && blockposition.getY() + i + 1 < 256) {
            ...
            for (int l = blockposition.getY(); l <= blockposition.getY() + 1 + i; ++l) {
                ...
                for (j = blockposition.getX() - b0; j <= blockposition.getX() + b0 && flag; ++j) {
                    for (k = blockposition.getZ() - b0; k <= blockposition.getZ() + b0 && flag; ++k) {
                        if (l >= 0 && l < 256) {
                            Block block = world.getType(j, l, k).getBlock();
            ...
            if (!flag) {
                ...
                if (block1 != Blocks.DIRT && block1 != Blocks.GRASS && block1 != Blocks.MYCELIUM) {
                        ...
                        for (int j2 = k; j2 <= k1; ++j2) {
                            for (int k2 = l1; k2 <= i2; ++k2) {
                                ...
                                if (blockposition.getY() >= blockposition.getY() + i - 1 || blockhugemushroom_enumhugemushroomvariant != BlockHugeMushroom.EnumHugeMushroomVariant.ALL_INSIDE) {
                                    if (!world.getType(j2, j1, k2).getBlock().o()) {
                                        this.a(world, new BlockPosition(j2, j1, k2), this.a.getBlockData().set(BlockHugeMushroom.VARIANT, blockhugemushroom_enumhugemushroomvariant));
                                    }
                                }
                            }
                        }
                    ...
                    for (j1 = 0; j1 < i; ++j1) {
                        BlockPosition up = blockposition.up(j1);
                        Block block2 = world.getType(up).getBlock();

                        if (!block2.o()) {
                            this.a(world, up, this.a.getBlockData().set(BlockHugeMushroom.VARIANT, BlockHugeMushroom.EnumHugeMushroomVariant.STEM));
                        }
                    }
