...
public class WorldGenMonument extends StructureGenerator {
    ...
    private int f;
    private int g;
    public static final List<BiomeBase> d = Arrays.asList(BiomeBase.OCEAN, BiomeBase.DEEP_OCEAN, BiomeBase.RIVER, BiomeBase.FROZEN_OCEAN, BiomeBase.FROZEN_RIVER);
    ...
    public WorldGenMonument(Map<String, String> map) {
        ...
        for (Entry<String, String> stringStringEntry : map.entrySet()) {
            ...
            if (stringStringEntry.getKey().equals("spacing")) {
                this.f = MathHelper.a(stringStringEntry.getValue(), this.f, 1);
            } else if (stringStringEntry.getKey().equals("separation")) {
                this.g = MathHelper.a(stringStringEntry.getValue(), this.g, 1);
            }
        }
    ...
    protected boolean a(int i, int j) {
        ...
        if (k == i1 && l == j1) {
            if (this.c.getWorldChunkManager().getBiome(new BlockPosition(k * 16 + 8, 64, l * 16 + 8), null) != BiomeBase.DEEP_OCEAN) {
                return false;
            }

            return this.c.getWorldChunkManager().a(k * 16 + 8, l * 16 + 8, 29, WorldGenMonument.d);
        }
