...
public final class CraftServer implements Server {
    ...
    private final String serverName = "InvictusSpigot";
    ...
    private final Spigot spigot = new Spigot() {
        ...
        public double[] getTPS() {
            return new double[]{
                    MinecraftServer.getServer().tps.getAverage(),
    ...
    public boolean dispatchCommand(CommandSender sender, String commandLine) {
        ...
        if (!Bukkit.isPrimaryThread()) {
            ...
            MinecraftServer.getServer().postToMainThread(wait);
            try {
                return wait.get();
    ...
    public void reload() {
        ...
        InvictusConfig.init((File) console.options.valueOf("invictus-settings"));
    ...
    public World createWorld(WorldCreator creator) {
        ...
        if (worlddata == null) {
            WorldSettings worldSettings = new WorldSettings(creator.seed(), WorldSettings.EnumGamemode.getById(getDefaultGameMode().getValue()), generateStructures, false, type);
            worldSettings.setGeneratorSettings(creator.generatorSettings());
            worlddata = new WorldData(worldSettings, name);
        }
        worlddata.checkName(name); // CraftBukkit - Migration did not rewrite the level.dat; This forces 1.8 to take the last loaded world as respawn (in this case the end)
        WorldServer internal = (WorldServer) new WorldServer(console, sdm, worlddata, dimension, getServer().methodProfiler, creator.environment(), generator).b();
    ...
    public Set<OfflinePlayer> getBannedPlayers() {
        ...
        for (JsonListEntry<GameProfile> entry : playerList.getProfileBans().getValues()) {
    ...
    public Set<OfflinePlayer> getWhitelistedPlayers() {
        ...
        for (JsonListEntry<GameProfile> entry : playerList.getWhitelist().getValues()) {
            result.add(getOfflinePlayer(entry.getKey()));
        }
    ...
    public Set<OfflinePlayer> getOperators() {
        ...
        for (JsonListEntry<GameProfile> entry : playerList.getOPs().getValues()) {
            result.add(getOfflinePlayer(entry.getKey()));
        }
    ...
    public List<String> tabCompleteChat(Player player, String message) {
        ...
        for (Player p : getOnlinePlayers()) {
            if (InvictusConfig.hidePlayersFromTab) {
