...
public class CraftLivingEntity extends CraftEntity implements LivingEntity {
    ...
    public void setHealth(double health) {
        if (health < 0.0D || health > getMaxHealth())
            throw new IllegalArgumentException("Health must be between 0 and " + getMaxHealth() + ", but was " + health + ". (attribute base value: " +
                    getHandle().getAttributeInstance(GenericAttributes.maxHealth).b() + ((this instanceof CraftPlayer) ? (", player: " +
                    getName() + ')') : Character.valueOf(')')));
    ...
    public boolean addPotionEffect(PotionEffect effect, boolean force) {
        return addPotionEffect(effect, force, PotionEffectAddEvent.EffectCause.PLUGIN);
    }

    public boolean addPotionEffect(PotionEffect effect, boolean force, PotionEffectAddEvent.EffectCause cause) {
        if (hasPotionEffect(effect.getType())) {
            if (!force)
                return false;
            removePotionEffect(effect.getType());
        }
        getHandle().addEffect(new MobEffect(effect.getType().getId(), effect.getDuration(), effect.getAmplifier(), effect.isAmbient(), effect.hasParticles()), cause);
        return true;
    }
