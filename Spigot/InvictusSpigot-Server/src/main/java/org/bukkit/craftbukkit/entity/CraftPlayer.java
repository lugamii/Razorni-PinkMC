...
public class CraftPlayer extends CraftHumanEntity implements Player {
	...
	private final Set<String> channels = new LinkedArraySet<>();
	...
	public void setVelocity(org.bukkit.util.Vector vel) {
		...
		if (!event.isCancelled()) {
			...
			setVelocityDirect(event.getVelocity()); // Directional kb
			...
			getHandle().playerConnection.sendPacket(new PacketPlayOutEntityVelocity(this.getHandle()));
		}
	...
	public boolean canSeeEntity(org.bukkit.entity.Entity entity) {
		...
		if (nmsEntity instanceof EntityProjectile) {
			...
			if (entityProjectile.getShooter() instanceof EntityPlayer)
				return getUniqueId().equals(entityProjectile.shooter.getUniqueID()) || canSee(((EntityPlayer)entityProjectile.getShooter()).getBukkitEntity());
			...
			if (entityItem.owner instanceof EntityPlayer)
				return getUniqueId().equals(entityItem.owner.getUniqueID()) || canSee(((EntityPlayer)entityItem.owner).getBukkitEntity());
			...
			if (entityProjectile.shooter instanceof EntityPlayer)
				return getUniqueId().equals(entityProjectile.shooter.getUniqueID()) || canSee(((EntityPlayer)entityProjectile.shooter).getBukkitEntity());
		}
	...
	public void setPlayerListName(String name) {
		...
		for (EntityPlayer player : this.server.getHandle().players) {
			if (!InvictusConfig.hidePlayersFromTab || player.getBukkitEntity().canSee(this))
	...
	public void hidePlayer(Player player) {
		...
		if (getHandle().playerConnection == null || equals(player) || hiddenPlayers.contains(player.getUniqueId())) {
			return;
		}
	...
	public void readExtraData(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKey("bukkit")) {
			...
			if(data.hasKey("profile")) {
				setKnockbackProfile(KnockbackAPI.getByName(data.getString("profile")));
			}
		}
	}
	...
	public void setExtraData(NBTTagCompound nbttagcompound) {
		...
		data.setString("profile", ((CraftKnockbackProfile) getHandle().getKnockbackProfile()).getName());
	}
	...
	private final Player.Spigot spigot = new Player.Spigot() {
		...
		public void playEffect(Location location, Effect effect, int id, int data, float offsetX, float offsetY,
			...
			if (effect.getType() != Effect.Type.PARTICLE) {
				...
				packet = new PacketPlayOutWorldEvent(packetData, new BlockPosition(location.getBlockX(), location.getBlockY(), location.getBlockZ()), id,
						false);
			} else {
				EnumParticle particle;
				int[] extra = null;
				if ((particle = EnumParticle.EFFECT_TO_PARTICLE.get(effect)) != null && effect.getData() != null) {
				...
				if (extra == null) {
					extra = EnumParticle.EMPTY_ARRAY;
				}
