...
import eu.vortexdev.invictusspigot.util.BoundingBox;
...
public class CraftEventFactory {
    ...
    public static PlayerInteractEvent callPlayerInteractEvent(EntityHuman who, Action action, BlockPosition position, EnumDirection direction, ItemStack itemstack, boolean cancelledBlock) {
        return callPlayerInteractEvent(who, action, who.getBukkitEntity().getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()), position, direction, itemstack, cancelledBlock);
    }

    public static PlayerInteractEvent callPlayerInteractEvent(EntityHuman who, Action action, Block blockClicked, BlockPosition position, EnumDirection direction, ItemStack itemstack, boolean cancelledBlock) {
        Player player = (who == null) ? null : (Player) who.getBukkitEntity();
        ...
        if (InvictusConfig.patchFreecamCegging && event.getAction() == Action.RIGHT_CLICK_BLOCK && !event.isCancelled()) {
            if ((event.getItem() != null && event.getItem().getType() == Material.MONSTER_EGG) || blockClicked.getState() instanceof org.bukkit.inventory.InventoryHolder) {
                Vector origin = player.getEyeLocation().toVector();
                Vector eyeDirection = player.getEyeLocation().getDirection();
                Block rayTraced = null;
                double d;
                for (d = 0.0D; d <= 5.0D; d += 0.01D) {
                    Block currentBlock;
                    Location currentLoc;
                    Material currentMat = (currentBlock = (currentLoc = origin.clone().add(eyeDirection.clone().multiply(d)).toLocation(player.getWorld())).getBlock()).getType();
                    if (isSolidBlock(currentBlock, currentMat, currentLoc)) {
                        rayTraced = currentBlock;
                        break;
                    }
                }
                if (!blockClicked.equals(rayTraced)) {
    ...
    private static boolean isSolidBlock(Block block, Material material, Location location) {
        if (material == Material.AIR || block.isLiquid() || block.getState() instanceof org.bukkit.block.Sign || block.getState().getData() instanceof org.bukkit.material.Stairs)
            return false;
        BoundingBox aabb = new BoundingBox(block);
        return (!(location.getX() < aabb.getMin().getX()) && !(location.getX() > aabb.getMax().getX())) && (!(location.getY() < aabb.getMin().getY()) && !(location.getY() > aabb.getMax().getY())) && (!(location.getZ() < aabb.getMin().getZ()) && !(location.getZ() > aabb.getMax().getZ()));
    }
     ...
     * BlockDamageEvent
     */
    public static BlockDamageEvent callBlockDamageEvent(EntityHuman who, Block block, ItemStack itemstack, boolean instaBreak) {
        Player player = (who == null) ? null : (Player) who.getBukkitEntity();
        BlockDamageEvent event = new BlockDamageEvent(player, block, CraftItemStack.asCraftMirror(itemstack), instaBreak);
    ...
    public static EntityChangeBlockEvent callEntityChangeBlockEvent(Entity entity, BlockPosition p, net.minecraft.server.Block type, int data) {
        return callEntityChangeBlockEvent(entity, p.getX(), p.getY(), p.getZ(), type, data);
    }
    ...
    public static BlockIgniteEvent callBlockIgniteEvent(World world, int x, int y, int z, Explosion explosion) {
        CraftWorld craftWorld = world.getWorld();
        CraftEntity craftEntity = (explosion.source == null) ? null : explosion.source.getBukkitEntity();
        BlockIgniteEvent event = new BlockIgniteEvent(craftWorld.getBlockAt(x, y, z), BlockIgniteEvent.IgniteCause.EXPLOSION, craftEntity);
        world.getServer().getPluginManager().callEvent(event);
        return event;
    }
    ...
    public static BlockIgniteEvent callBlockIgniteEvent(World world, Chunk chunk, int x, int y, int z, Explosion explosion) {
        org.bukkit.World bukkitWorld = world.getWorld();
        org.bukkit.entity.Entity igniter = explosion.source == null ? null : explosion.source.getBukkitEntity();

        BlockIgniteEvent event = new BlockIgniteEvent(chunk.bukkitChunk.getBlock(x, y, z), IgniteCause.EXPLOSION, igniter);
        world.getServer().getPluginManager().callEvent(event);
        return event;
    }
